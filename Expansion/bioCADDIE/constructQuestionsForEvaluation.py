...
files= ['query1.sparql', 'query2.sparql', 'query3.sparql', 'query4.sparql', 'query5.sparql', 'query6.sparql', 'query7.sparql', 'query8.sparql', 'query9.sparql', 'query10.sparql', 'query11.sparql', 'query12.sparql', 'query13.sparql', 'query14.sparql','query15.sparql']
...
instPattern = "({(Organism|Process|Disease|GeneProtein|Anatomy|Material)\s?inst\s?=\s?\"(http://purl.obolibrary.org/obo/[A-Za-z]+_[A-Z0-9]+)\"})"
	...
	try:
		...
		for j,fileLine in enumerate(lines): 
			...
			if count < 1 or fileLine == '' or fileLine.startswith('_:node') or fileLine == 'broader':
				...
				#if expansion == 'broader':
					#replacement = replacement + "{"+category+" inst=\""+fileLine+"\"} OR "
